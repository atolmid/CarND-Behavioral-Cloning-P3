...
def load__add_data(samples):
    ...
    for batch_sample in samples:
        try:
            i+=1
            name = './record/IMG/'+batch_sample[0].split('/')[-1]
            #print("name : ", name)
            center_image = cv2.imread(name)
            center_angle = float(batch_sample[3])
            images.append(center_image)
            angles.append(center_angle)
            name1 = './record/IMG/'+batch_sample[1].split('/')[-1]
            #print("name1 : ", name1)
            left_image = cv2.imread(name1)
            images.append(left_image)
            angles.append(center_angle+correction)
            name2 = './record/IMG/'+batch_sample[2].split('/')[-1]
            #print("name1 : ", name2)
            right_image = cv2.imread(name2)
            images.append(right_image)
            angles.append(center_angle-correction)
...
def combine(images, angles, images1, angles1):
    for image, angle in zip(images1, angles1):
        images.append(image)
        angles.append(angle)
    return images, angles


#add_im, add_ang = load__add_data(samples1)

train_images, train_angles = load__add_data(samples1)
